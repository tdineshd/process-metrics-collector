...
if [ -z $1 ] | [ -z $2 ]; then
  ...
  echo
  echo "Usage: $(basename "$0") <PID> <PID2>"
  exit 1
fi
...
if [ $pid2_exist != 0 ]; then
  echo "ERROR: Process ID $pid2 not found."
  exit 1
fi

current_time=$(date +"%Y_%m_%d_%H%M")
dir_name="data/${pid}-${pid2}-${current_time}"
...
function plotGraph() {
  ...
  if [ -f $csv_filename ]; then
    ...
    gnuplot <<- EOF
      ...
      set title "CPU Usage for Proces ID Python $pid C++ $pid2"
      plot "$csv_filename" using 5:xticlabels(1) with lines smooth unique lw 2 lt rgb "tan1" t "Python CPU Usage %",\
       "$csv_filename" using 13:xticlabels(1) with lines smooth unique lw 2 lt rgb "skyblue" t "C++ CPU Usage %"

      set output "${dir_name}/mem-usage-percent.png"
      set title "Memory Usage for Proces ID Python $pid C++ $pid2"
      plot "$csv_filename" using 6:xticlabels(1) with lines smooth unique lw 2 lt rgb "dark-plum" t "Python Memory Usage %",\
       "$csv_filename" using 14:xticlabels(1) with lines smooth unique lw 2 lt rgb "blue" t "C++ Memory Usage %"

      # Memory usage
      set output "${dir_name}/mem-usage.png"
      set title "Memory Usage for Proces ID Python $pid C++ $pid2"
      plot "$csv_filename" using 3:xticlabels(1) with lines smooth unique lw 2 lt rgb "tan1" t "PythonRES",\
       "$csv_filename" using 11:xticlabels(1) with lines smooth unique lw 2 lt rgb "blue" t "C++ RES"

      # TCP count
      set output "${dir_name}/tcp-count.png"
      set title "TCP Connections Count for Proces ID Python $pid C++ $pid2"
      ...
      set title "Thread Count for Proces ID Python $pid C++ $pid2"
      plot "$csv_filename" using 8:xticlabels(1) with lines smooth unique lw 2 lt rgb "tan1" t "Python Thread Count",\
       "$csv_filename" using 16:xticlabels(1) with lines smooth unique lw 2 lt rgb "blue" t "C++ Thread Count"

       # All together
       set output "${dir_name}/all-metrices.png"
       set title "All Metrics for Proces ID Python $pid C++ $pid2"
...
# write CSV headers
echo "Time,VIRT, RES, SHR, CPU,Memory,TCP Connections,Thread Count, ,VIRT, RES, SHR, CPU,Memory,TCP Connections,Thread Count" >> $csv_filename
...
while [ $pid_exist == 0 ] && [ $pid2_exist == 0 ]; do
  # check if process exists
  kill -0 $pid > /dev/null 2>&1
  pid_exist=$?
  ...
  if [ $pid_exist == 0 ] && [ $pid2_exist == 0 ]; then
    ...
    cpu_mem_usage2=$(top -b -n 1 | grep -w -E "^ *$pid2" | awk '{print $5 "," $6 "," $7 "," $9 "," $10}')
    tcp_cons2=$(lsof -i -a -p $pid2 -w | tail -n +2 | wc -l)
    tcount2=$(ps -o nlwp h $pid2 | tr -d ' ')

    # write CSV row
    echo "$timestamp,$cpu_mem_usage,$tcp_cons,$tcount,$pid2,$cpu_mem_usage2,$tcp_cons2,$tcount2" >> $csv_filename
    sleep 1
  fi
done
